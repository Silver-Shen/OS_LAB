...
alloc_proc(void) {
	...
	if (proc != NULL) {
		 ...
		 */
		//LAB8:EXERCISE2 2013011371 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
	...
	//LAB8:EXERCISE2 2013011371  HINT:how to copy the fs in parent's proc_struct?
	/*
	...
	if (setup_kstack(proc) != 0){
		goto bad_fork_cleanup_proc; //step 2
	}
	...
	if (copy_mm(clone_flags, proc) != 0) {
		goto bad_fork_cleanup_fs; //step 3
	}
...
load_icode(int fd, int argc, char **kargv) {
	/* LAB8:EXERCISE2 2013011371  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
	 * MACROs or Functions:
